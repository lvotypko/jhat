...
public class Snapshot {
    ...
    private List<JavaHeapObject> unreachableObjects = new ArrayList<JavaHeapObject>();
    ...
    public void resolve(boolean calculateRefs) {
        ...
        if (calculateRefs) {
            findUnreachableObjectsFormRoots();
        }
    ...
    public void findUnreachableObjectsFormRoots(){
        List<JavaHeapObject> h = new ArrayList<JavaHeapObject>();
        for(JavaHeapObject obj : heapObjects.values()){
            if(rootsMap.get(obj)==null){
    ...
    public List<JavaHeapObject> getUnreachableObjects(){
        return Collections.unmodifiableList(unreachableObjects);
    }
    
    public List<JavaClass> findAllSuperClass(JavaHeapObject object){
        JavaClass clazz = object.getClazz();
        clazz = clazz.getSuperclass();
        List<JavaClass> superClasses = new ArrayList<JavaClass>();
        while(clazz!=null){
    ...
    public List<JavaHeapObject> getUnreachableObjects(String classFullName, boolean includeSubclass){
        List<JavaHeapObject> objects = new ArrayList<JavaHeapObject>();
        for(JavaHeapObject obj : unreachableObjects){
            if(classFullName.equals(obj.getClazz().getName())){
                objects.add(obj);
                continue;
            }
            if(includeSubclass){
                for(JavaClass c : findAllSuperClass(obj)){
                    if(classFullName.equals(c.getName())){
                        objects.add(obj);
                    }
                }
            }
