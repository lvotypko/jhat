...
public class HprofReader extends Reader /* imports */ implements ArrayTypeCodes {
    ...
    private boolean readCorruptedHprof;
    
    private long skippedErrorBytes = 0;

    public HprofReader(String fileName, PositionDataInputStream in,
                       int dumpNumber, boolean callStack, int debugLevel, boolean readCorruptedHprof)
                       throws IOException {
        super(in);
        ...
        this.readCorruptedHprof = readCorruptedHprof;
                ...
                try {
                   ...
                   // System.out.println("type is " + type + " and current position " + currPos);
                    ...
                    if(readCorruptedHprof){
                        //mark this part for case the hprof is corrupted
                        in.mark();
                    }
    ...
    private void readHeapDump(long bytesLeft, long posAtEnd) throws IOException {
        ...
        while (bytesLeft > 0) {
            ...
            long cachedBytesLeft = bytesLeft;
            try{
                if (debugLevel > 0) {
                ...
                switch(type) {
                    ...
                    default: {
                        if(readCorruptedHprof){
                            skippedErrorBytes++;
                            bytesLeft = cachedBytesLeft + in.backOnMarkedPosition();
                        }
                        else{
                ...
                if(readCorruptedHprof){
                    skippedErrorBytes++;
                    bytesLeft = bytesLeft + in.backOnMarkedPosition();
                }
                else{
                    throw new Error(e);
                }
