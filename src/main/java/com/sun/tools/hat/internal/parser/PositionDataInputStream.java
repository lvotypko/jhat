...
public class PositionDataInputStream extends DataInputStream {
    public PositionDataInputStream(InputStream in, int byteBuffer) {
        super(in instanceof PositionInputStream?
              in : new PositionInputStream(in, byteBuffer));
    }
    ...
    public synchronized long backOnMarkedPosition() throws IOException {
        if(in.markSupported()){
            return ((PositionInputStream)in).backOnMarkedPosition();
        }
        return 0;
    }
    
    public synchronized void mark(){
        ((PositionInputStream)in).mark();
    }
