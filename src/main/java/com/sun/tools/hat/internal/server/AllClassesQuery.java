...
class AllClassesQuery extends QueryHandler {
    ...
    public void run() {
        ...
        if(snapshot.skippedBytes>0){
            out.println("<br>");
            out.println("<style> h4 {color:red;} </style>");
            out.println("<h4>Hprof file is probably corrupted, " + snapshot.skippedBytes + " bytes had to be skipped</h4>");
        }
        ...
        out.println("<li>");
        printAnchorStart();
        out.print("unreachableObjects/\">");
        print("Show unreachable objects from roots");
        out.println("</a>");
