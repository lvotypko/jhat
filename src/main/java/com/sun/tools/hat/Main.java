...
public class Main {
    ...
    private static void usage(String message) {
        ...
        System.err.println("\t-CorrupParserBuffer <size>    Enable parsing corrupt dump.");
        System.err.println("\t\t\t  Set max size of available buffer (in bytes) for work with bytes");
        System.err.println("\t\t\t  This buffer has to have at least the size of the largest object.");
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; ; i += 2) {
            ...
            } else if("-CorrupParserBuffer".equals(key)){
                byteBuffer = Integer.parseInt(value, 10);
            }
        }
        ...
        try {
            model = com.sun.tools.hat.internal.parser.Reader.readFile(fileName, callStack, debugLevel,byteBuffer);
        ...
        //pridano
        //System.out.println("objekty " + model.heapObjects.size());
        if (excludeFile != null) {
            model.setReachableExcludes(new ReachableExcludesImpl(excludeFile));
        }
        // System.out.println("objects all heap");
        //for (JavaHeapObject o : model.heapObjects.values()){
           
        
        //System.out.println("find 33671532632" + model.findThing(33671532632l));
